...
<body>
	<div id="container">
		...
		<div id="welcome-container">
			<p id="user-welcome"><%=userNickname%>님, 문제 생성을 위해 카테고리를 골라주세요.
			</p>
		</div>
		<div id="sub-container-row">
			...
			<div id="describe-container">
				<p id="describe">추가하고 싶은 노래가 있다면, 직접 추가합니다</p>
			</div>
		</div>
	</div>
	<script>
		...
		function sendPostRequest(buttonId) {
			...
			form.setAttribute("action", "createCategory"); // 실제 서블릿 이름으로 변경해야 합니다.
		...
		lyricsButton.addEventListener("click", function() {
			displayText("추가하고 싶은 노래가 있다면, 직접 추가합니다");
		});

		wordsButton.addEventListener("click", function() {
			displayText("추가하고 싶은 단어가 있다면, 직접 추가합니다");
		});

		adviceButton.addEventListener("click", function() {
			displayText("추가하고 싶은 명언이 있다면, 직접 추가합니다");
		});
